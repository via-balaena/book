...
| **Applications are part of the kernel image.** An application's [TBF](../doc/tock_binary_format.md) is copied into the kernel ELF's `.apps` section.                    | Loading an application reflashes the kernel with it, so the [getting started guide](../getting_started.md)'s promise that you can install apps without having to update or re-flash the kernel does not hold. This page does not cover applications; see [Where to go next](#where-to-go-next). |
